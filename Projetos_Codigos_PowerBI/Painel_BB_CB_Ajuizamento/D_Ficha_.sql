...
LEFT JOIN (
    ...
    ROW_NUMBER() OVER (PARTITION BY PASTA ORDER BY DATA_LIMITE_COMPROMISSO DESC) AS rn
    FROM Juridico.Compromissos
    WHERE TIPO_COMPROMISSO = 'RECOLHER TAXA DE DISTRIBUIÇÃO DOS AUTOS'
...
LEFT JOIN (
    ...
    ROW_NUMBER() OVER (PARTITION BY PASTA ORDER BY DATA_LIMITE_COMPROMISSO DESC) AS rn
    FROM Juridico.Compromissos
    WHERE TIPO_COMPROMISSO = 'CORRIGIR RECOLHIMENTO DE GUIAS'
...
AND FP.TIPO_ACAO NOT IN (
    'CARTA PRECATÓRIA', 'EMBARGOS', 'EMBARGOS DE TERCEIRO', 'FALÊNCIA',
    'EMBARGOS A EXECUÇÃO FISCAL', 'TRABALHISTA', 'RECUPERAÇÃO JUDICIAL',
    'RENOVÁTORIA DE LOCAÇÃO', 'RESCISÓRIA', 'INSOLVÊNCIA CIVIL',
    'PROTESTO POR PREFERENCIA', 'RECLAMATÓRIA TRABALHISTA',
    'EMBARGOS A EXECUÇÃO', 'EMBARGOS A ARREMATAÇÃO', 'FALENCIA',
    'RECUPERACAO JUDICIAL', 'CUMPRIMENTO DE SENTENÇA'
)
AND FP.AREA != 'CONTENCIOSO CIVEL'
AND FP.DEPARTAMENTO = 'COBRANÇA'
AND FP.CASO_NOVO = 'SIM'
AND FP.FASE != 'RECUPERAÇÃO JUDICIAL'
...
AND (
    ...
    OR (
        ...
        AND FP.PASTA IN (
            ...
            AND ANDA.DATA_ANDAMENTO >= '01/01/2019'
        )
    )
)
AND (
